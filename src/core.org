    ...
    (let [type-name (symbol (str "NDArray" dim (name type-id)))
      ...
      `(do
         (deftype ~type-name
           ...
           (~'set-at
             [_# ~@psyms ~c]
             (~set ~(type-hinted type-hint '_data) (int ~idx-syms) ~(if cast `(~cast ~c) c)) _#)
           (~'set-at-safe
             [_# ~@psyms ~c]
             ~(with-bounds-check dim psyms shapes clj?
           ...
           (~'update-at
             ...
             (let [~c (int ~idx-syms)]
               (~set ~(type-hinted type-hint '_data) ~c
                     ~(if cast
                        `(~cast (~f ~@psyms (~get ~(type-hinted type-hint '_data) ~c)))
                        `(~f ~@psyms (~get ~(type-hinted type-hint '_data) ~c)))))
             _#)
           (~'update-at-safe
             [_# ~@psyms ~f]
             ~(with-bounds-check dim psyms shapes clj?
