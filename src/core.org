...
#+BEGIN_SRC clojure :noweb-ref ctors
  ...
  #?(:clj
     (do
       ...
       (def-ndarray 2 short   "shorts"   :int16   short-array aget aset-short true)
       ...
       (def-ndarray 3 short   "shorts"   :int16   short-array aget aset-short true)
       ...
       (def-ndarray 4 short   "shorts"   :int16   short-array aget aset-short true)
      ...
      `(do
         (deftype ~type-name
           ...
           (~'extract
             ...
             (let [buf#      ~(type-hinted type-hint `(~data-ctor (* ~@(pair-fn '* shapes))))
               ...
               (loop [~c (thi.ng.ndarray.core/index-seq _#)
                 ...
                 (when ~c
                   (~set buf# (int (first ~d)) ~(do-cast cast `(~get ~data (first ~c))))
                   (recur (next ~c) (next ~d))))
               arr#))
           (~'index-at
             [_# ~@psyms] ~idx-syms)
           ...
           (~'update-at-safe
             ...
             _#)
           (~'update-at-index
             [_# ~c ~f] (~set ~data ~c ~(do-cast cast `(~f ~c (~get ~data ~c)))) _#)
           (~'truncate-h
             [_# ~@psyms]
           ...
           (~'toString
             ...
             (pr-str
              {:data ~data :type ~type-id
               :size (* ~@(pair-fn '* shapes)) :total (count ~data) :offset ~'_offset
